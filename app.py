...
def load_data():
    ...
    if GEOPANDAS_AVAILABLE:
        map_data_path = Path('Uganda map data/ug_districts.gpkg')
        if map_data_path.exists():
            try:
                uganda_map = gpd.read_file(str(map_data_path))
                if 'DISTRICT' in uganda_map.columns:
                    uganda_map['DISTRICT'] = uganda_map['DISTRICT'].str.upper().str.strip()
                    vulnerability['Region'] = vulnerability['Region'].str.upper().str.strip()
                    merged_data = uganda_map.merge(
                        vulnerability, left_on='DISTRICT', right_on='Region', how='left'
                    )
                    return vulnerability, nutrients, merged_data
                else:
                    st.error("Map file loaded but 'DISTRICT' column missing. Please check the map data file.")
            except Exception as e:
                st.error(f"Could not load map data: {str(e)}. Please ensure 'Uganda map data/ug_districts.gpkg' is valid.")
        else:
            st.warning("Map file 'Uganda map data/ug_districts.gpkg' not found. Map visualization will be limited.")
    else:
        st.warning("GeoPandas is not available. Map visualization will be limited.")
    
    return vulnerability, nutrients, None
...
# ---------------- ENHANCED RISK MAP ---------------- #
    ...
    if uganda_map is not None and not uganda_map.empty:
        # Choropleth map
        fig = px.choropleth(
            uganda_map,
            geojson=uganda_map.geometry,
            locations=uganda_map.index,
            color=vuln_type,
            hover_name="DISTRICT",
            hover_data={
                vuln_type: ":.2f",
            ...
            title=f"🌍 {vuln_type} Across Uganda Districts"
        )
        fig.update_geos(
            fitbounds="locations",
            visible=False,
